    ...
    public class MySQL
    {
        ...
        public async Task<bool> PerformTransaction(Func<MySqlConnection, Task<bool>> transaction)
        {
            ...
            using (MySqlConnection cnn = new MySqlConnection(ConnectionString))
            {
                ...
                result = await transaction(cnn).ConfigureAwait(false);
            }
            return result;
        }

        public async Task PerformTransaction(Func<MySqlConnection, decimal, Task> transaction, decimal c)
        {
            using (MySqlConnection cnn = new MySqlConnection(ConnectionString))
            {
                await cnn.OpenAsync().ConfigureAwait(false);
                await transaction(cnn, c).ConfigureAwait(false);
            }
        }
